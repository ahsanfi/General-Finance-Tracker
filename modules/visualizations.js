...
(() => {
  ...
  function spending(items, currency, comparison) {
    ...
    host.innerHTML = `<svg class="spending-arcs" viewBox="0 0 620 350" role="group" aria-label="Spending by category">${arcs}<text x="310" y="150" text-anchor="middle" class="radial-caption">TOTAL SPENDING</text><text x="310" y="177" text-anchor="middle" class="radial-total">${money(total,currency)}</text><text x="310" y="199" text-anchor="middle" class="radial-caption">All recorded expenses</text></svg><div class="spending-mobile"><span class="visual-caption">Total spending · all time</span><strong class="mobile-spending-total">${money(total,currency)}</strong>${visible.map((item,i)=>`<div class="direct-category"><span class="direct-category-name">${escape(item.name)}</span><strong>${money(item.value,currency)}</strong><span>${percent(item.value,total).toFixed(1)}%</span><div class="direct-category-track"><span style="width:${percent(item.value,total)}%;background:${colors[i]}"></span></div></div>`).join('')}</div><p class="spending-comparison">${escape(comparison)}</p>${details(items,total,currency,false,open)}`;
  }
  ...
  FinTracker.visualizations={spending,portfolio,summarize,split};
})();
